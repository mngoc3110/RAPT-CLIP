...
def preprocess_image(img_bgr, num_segments=16):
    """Convert BGR image to CLIP input tensor format [1, T, C, H, W] matching training."""
    ...
    tensor = torch.tensor(img_chw, dtype=torch.float32)
    
    # Critical Fix: Model was trained on 16 frames. Feeding 1 frame causes the temporal 
    # transformer's positional embeddings to mess up the features. Replicating the frame 16 times.
    tensor = tensor.unsqueeze(0).repeat(num_segments, 1, 1, 1) # Shape: [16, 3, 224, 224]
...
def main():
    ...
    parser.add_argument("--checkpoint", type=str, default="outputs/RAER-ramp-down/model_best.pth")
    ...
    if not cap.isOpened(): 
        print(f"Cannot open camera {args_cmd.camera_id}")
        return
